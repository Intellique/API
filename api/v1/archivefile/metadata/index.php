<?php
/**
 * To delete all metadata of an archivefile
 * To delete metadata by the key : "name", "format", "observation", "conserver jusqu'au" of an archivefile
 * use \b DELETE method : <i>with a reference to an archivefile id and or the key(s) of the metadatas.
...
case 'DELETE':
	...
	$permission = $dbDriver->checkArchiveFilePermission($_GET['id'], $_SESSION['user']['id']);
	if ($permission === null) {
	...
	if (!$dbDriver->startTransaction()) {
		$dbDriver->writeLog(DB::DB_LOG_EMERGENCY, sprintf('DELETE api/v1/archivefile/metadata (%d) => Failed to start transaction', __LINE__), $_SESSION['user']['id']);
	...
	if (isset($_GET['key'])) {
		...
		$delete_metadata = $dbDriver->deleteMetadata($_GET['id'], $key, 'archivefile', $_SESSION['user']['id']);
		...
		$dbDriver->writeLog(DB::DB_LOG_INFO, sprintf('DELETE api/v1/archivefile/metadata => metadata %s deleted', $_GET['id']), $_SESSION['user']['id']);
		...
		$delete_metadata = $dbDriver->deleteMetadatas($_GET['id'], 'archivefile', $_SESSION['user']['id']);
