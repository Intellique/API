...
class MediaFormatTest(CommonTest):
    def test_01_post_not_logged(self):
        ...
        conn.request('POST', self.path + 'archive/import/')
    ...
    def test_02_post_admin_user_with_no_media_id(self):
        ...
        conn.request('POST', self.path + 'archive/import/', headers=headers)
        res = conn.getresponse()
        conn.close()
    ...
    def test_03_post_admin_user_with_wrong_media_id(self):
        ...
        headers = {"Content-type": "application/json"}
        headers.update(cookie)
        conn.request('POST', self.path + 'archive/import/', body=data, headers=headers)
        res = conn.getresponse()
        conn.close()
        self.assertEqual(res.status, 400)
